...
$warehouse_webroot = $registry->get('webroot');
$horde_webroot = $registry->get('webroot', 'horde');
$has_tasks = Warehouse::hasApiPermission('tasks');

/* Variables used in core javascript files. */
$code['conf'] = array(
    'URI_AJAX' => (string)Horde::getServiceLink('ajax', 'warehouse'),
    ...
    'prefs_url' => (string)Horde::getServiceLink('prefs', 'warehouse')->setRaw(true)->add('ajaxui', 1),
    'name' => $registry->get('name'),
    'has_tasks' => $has_tasks,
    'default_calendar' => 'internal|' . Warehouse::getDefaultCalendar(Horde_Perms::EDIT),
    ...
    'status' => array('tentative' => Warehouse::STATUS_TENTATIVE,
                      'confirmed' => Warehouse::STATUS_CONFIRMED,
                      'cancelled' => Warehouse::STATUS_CANCELLED,
                      'free' => Warehouse::STATUS_FREE),
    ...
    'perms' => array('all' => Horde_Perms::ALL,
                     ...
                     'delegate' => Warehouse::PERMS_DELEGATE),
    ...
    foreach ($GLOBALS['all_calendars'] as $id => $calendar) {
        ...
        if (($my && $owner) || (!$my && !$owner)) {
            $code['conf']['calendars']['internal'][$id] = array(
                ...
                'feed' => (string)Warehouse::feedUrl($id),
                'embed' => Warehouse::embedCode($id));
            if ($owner) {
                $code['conf']['calendars']['internal'][$id]['perms'] = Warehouse::permissionToJson($calendar->share()->getPermission());
            }
        }
    }
    ...
    foreach ($registry->tasks->listTasklists($my, Horde_Perms::SHOW) as $id => $tasklist) {
        ...
        if (($my && $owner) || (!$my && !$owner)) {
            $code['conf']['calendars']['tasklists']['tasks/' . $id] = array(
                ...
                'owner' => $owner,
                'fg' => Warehouse::foregroundColor($tasklist),
                'bg' => Warehouse::backgroundColor($tasklist),
            ...
            if ($owner) {
                $code['conf']['calendars']['tasklists']['tasks/' . $id]['perms'] = Warehouse::permissionToJson($tasklist->getPermission());
            }
        }
    }
}
